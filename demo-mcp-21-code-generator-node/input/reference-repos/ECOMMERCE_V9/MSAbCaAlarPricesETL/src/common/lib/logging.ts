...
export default class Logging {
  ...
  public write(
    message: string,
    taks: any,
    level: ELevelsErros = ELevelsErros.INFO,
    request?: any,
    response?: any,
    ...
    if (
      this.LOG_LEVEL === ELevelsErrosGlobal.ALL ||
      this.LOG_LEVEL === ELevelsErrosGlobal.INFO ||
      (this.LOG_LEVEL === ELevelsErrosGlobal.WARNING &&
        LEVEL !== ELevelsErrosGlobal.INFO) ||
      (this.LOG_LEVEL === ELevelsErrosGlobal.ERROR &&
        LEVEL === ELevelsErrosGlobal.ERROR)
    ) {
      logger.log(
        logOutput(
          info.name,
