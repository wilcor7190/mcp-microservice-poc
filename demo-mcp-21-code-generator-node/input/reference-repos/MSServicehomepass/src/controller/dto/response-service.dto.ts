...
export class ResponseService<T = any> {
  ...
  constructor(success: boolean = true, message: EmessageMapping | string = EmessageMapping.DEFAULT, status: number = 200, documents?: T) {
    this.process = utils.getCorrelationalId;
    this.success = success;
    this.status = status;
    this.documents = documents;
    this.message = utils.mappingMessage(EmessageMapping[message]) || message;
  }
}
